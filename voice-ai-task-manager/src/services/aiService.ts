...
export class AIService {
  ...
  async processMessage(
    ...
    const triggerDetected = this.detectTriggerPhrases(userInput, conversationFlow.triggerPhrases);
    ...
    const newStage = this.updateConversationStage(userInput, currentStage, triggerDetected, extractedTasks.length > 0);
    
    // Generate response based on current stage
    const response = this.generateStageAwareResponse(userInput, intent, extractedTasks, conversationHistory, newStage);
    ...
    } else if (newStage.stage === 'task-analysis' || triggerDetected?.type === 'taskAnalysis') {
  ...
  private updateConversationStage(
    ...
    } else if (newStage.messageCount >= 2 && (hasNewTasks || newStage.topicFocus === 'work')) {
    ...
    } else if (newStage.messageCount >= 4 && newStage.stage === 'rapport-building') {
  ...
  private detectUserEnergyLevel(input: string): 'high' | 'medium' | 'low' {
    const lowerInput = input.toLowerCase();
    const highEnergyWords = ['excited', 'awesome', 'amazing', 'fantastic', 'perfect', 'incredible', 'love'];
    const lowEnergyWords = ['tired', 'overwhelmed', 'stressed', 'busy', 'exhausted', 'struggling'];
    
    const hasHighEnergy = highEnergyWords.some(word => lowerInput.includes(word));
  ...
  private detectTopicFocus(input: string): 'casual' | 'work' | 'planning' {
    const lowerInput = input.toLowerCase();
    const workWords = ['project', 'task', 'work', 'deadline', 'meeting', 'development'];
    const planningWords = ['plan', 'schedule', 'organize', 'goal', 'strategy', 'timeline'];
    
    const hasWorkFocus = workWords.some(word => lowerInput.includes(word));
    const hasPlanningFocus = planningWords.some(word => lowerInput.includes(word));
  ...
  private generateStageAwareResponse(
    ...
    if (stage.stage === 'rapport-building') {
      return this.generateRapportBuildingResponse(input, intent, extractedTasks, history, stage);
    } else if (stage.stage === 'task-analysis') {
      return this.generateTaskAnalysisResponse(input, intent, extractedTasks, history, stage);
    } else {
      return this.generateMixedResponse(input, intent, extractedTasks, history, stage);
    }
  }
  ...
  private generateRapportBuildingResponse(
    ...
    let response = this.buildRapportResponse(input, intent, userEnergy, stage.messageCount);
  ...
  private generateMixedResponse(
    ...
    const baseResponse = this.generateResponse(input, intent, extractedTasks, history);
    const rapportElement = this.generateEnergyMatchedFollowUp(stage.userEnergyLevel, stage.topicFocus);
    
    return baseResponse + ` ${rapportElement}`;
  }

  private buildRapportResponse(input: string, intent: string, userEnergy: string, messageCount: number): string {
    ...
    const responses = energyResponses[userEnergy as keyof typeof energyResponses] || energyResponses.medium;
    return responses[Math.floor(Math.random() * responses.length)];
  }

  private generateEnergyMatchedFollowUp(userEnergy: string, topicFocus: string): string {
    const followUps = {
      high: {
    ...
    const energyFollowUps = followUps[userEnergy as keyof typeof followUps] || followUps.medium;
    return energyFollowUps[topicFocus as keyof typeof energyFollowUps] || energyFollowUps.casual;
  }
  ...
  private generateResponse(
    ...
    const insights = this.generateProductivityInsights(extractedTasks, recentHistory, workPatterns);
    const strategicSuggestions = this.generateStrategicSuggestions(extractedTasks, conversationThemes);
    const followUpQuestions = this.generateFollowUpQuestions(intent, extractedTasks, conversationThemes);

    let response = this.buildMainResponse(intent, extractedTasks, hasRecentContext, conversationThemes, lastAssistantMessage);

    // Add detailed task analysis if tasks were extracted
    if (extractedTasks.length > 0) {
      response += this.buildTaskAnalysis(extractedTasks, workPatterns, hasRecentContext);
    }
    ...
    if (this.userMemory.contextualInfo.currentProjects.length > 0) {
      response += this.buildProjectContext(extractedTasks, conversationThemes, hasRecentContext);
    }
  ...
  private buildTaskAnalysis(extractedTasks: Partial<Task>[], workPatterns: any, hasRecentContext: boolean): string {
  ...
  private generateProductivityInsights(extractedTasks: Partial<Task>[], history: Message[], workPatterns: any): string[] {
    ...
    if (extractedTasks.length > 0) {
      ...
      if (currentHour < 10 && extractedTasks.some(t => t.priority === 'high')) {
        insights.push("I notice you're planning high-priority work early in the day - that's typically when cognitive energy is highest!");
      } else if (currentHour > 15 && extractedTasks.some(t => t.priority === 'urgent')) {
        insights.push("You're identifying urgent tasks later in the day. Consider if any of these could be tackled tomorrow morning when you're fresh.");
      }
    }
    
    // Analyze conversation patterns
    if (history.length > 5) {
      const recentTaskCounts = history.slice(-5).map(m => m.extractedTasks?.length || 0);
      const avgTasksPerMessage = recentTaskCounts.reduce((a, b) => a + b, 0) / recentTaskCounts.length;
      
      if (avgTasksPerMessage > 2) {
        insights.push("You've been generating a lot of tasks in our conversation. That's great planning, but consider prioritizing which ones to tackle first.");
      }
    }
    
    // Analyze work patterns
    const hasDeadlinePattern = extractedTasks.some(t => t.dueDate) || history.some(m => m.content.toLowerCase().includes('deadline'));
    if (hasDeadlinePattern) {
      insights.push("I see you're thinking about deadlines - would it help to map out a timeline and work backwards from your due dates?");
    }

    return insights;
  }

  private generateStrategicSuggestions(extractedTasks: Partial<Task>[], conversationThemes: string[]): string[] {
    const suggestions: string[] = [];
    
    if (extractedTasks.length > 3) {
      suggestions.push("With multiple tasks identified, consider using time-blocking to batch similar activities together for better focus.");
    }
    
    if (conversationThemes.includes('project') && extractedTasks.length > 1) {
      suggestions.push("For project work like this, I often recommend the 'cascade approach' - identify which tasks unlock others and sequence them strategically.");
    }
    
    if (conversationThemes.includes('deadline') || extractedTasks.some(t => t.priority === 'urgent')) {
      suggestions.push("Given the time sensitivity, consider identifying the minimum viable scope for each task - what's the 80% solution that gets you to done?");
    }
    
    if (conversationThemes.includes('meeting') && extractedTasks.length > 0) {
      suggestions.push("Since meetings are involved, consider preparing agenda items or follow-up tasks in advance to maximize the collaborative time.");
    }

    return suggestions;
  }

  private generateFollowUpQuestions(intent: string, extractedTasks: Partial<Task>[], conversationThemes: string[]): string[] {
    const questions: string[] = [];
    
    if (intent === 'task_creation' && extractedTasks.length > 0) {
      if (extractedTasks.some(t => !t.dueDate)) {
        questions.push("Are there any specific deadlines or time constraints I should know about for these tasks?");
      }
      
      if (extractedTasks.some(t => !t.project)) {
        questions.push("Do these tasks belong to a particular project or initiative you're working on?");
      }
      
      if (extractedTasks.length > 2) {
        questions.push("Which of these tasks would you like to tackle first, or should we prioritize them based on dependencies?");
      }
    }
    
    if (intent === 'project_discussion') {
      questions.push("What's the ultimate goal or outcome you're aiming for with this project?");
      questions.push("Are there any dependencies or constraints that might affect how we sequence these tasks?");
    }
    
    if (conversationThemes.includes('deadline')) {
      questions.push("How much time do you typically like to build in as a buffer before deadlines?");
    }
    
    if (conversationThemes.includes('meeting')) {
      questions.push("What outcomes do you want to achieve from these meetings, and what preparation would make them most effective?");
    }

    return questions.slice(0, 2); // Return up to 2 follow-up questions
  }

  private buildProjectContext(extractedTasks: Partial<Task>[], conversationThemes: string[], hasRecentContext: boolean): string {
    const currentProjects = this.userMemory.contextualInfo.currentProjects;
    let context = "";
    
    if (currentProjects.length > 0) {
      const relevantProject = currentProjects.find(project => 
        extractedTasks.some(task => task.project?.toLowerCase().includes(project.toLowerCase())) ||
      ...
      if (hasRecentContext) {
        context += "Based on our conversation, I'm starting to see how these pieces fit together strategically.";
      } else {
        context += "Would it be helpful to explore how this fits into your broader project goals?";
      }
    }
  ...
  private analyzeWorkPatterns(history: Message[]): any {
    ...
    if (content.includes('detail') || content.includes('specific') || content.includes('exactly')) {
      patterns.planningStyle = 'detailed';
    } else if (content.includes('quick') || content.includes('rough') || content.includes('general')) {
      patterns.planningStyle = 'high-level';
    }
    
    // Analyze urgency patterns
    const urgencyWords = content.match(/urgent|asap|immediately|rush|deadline/g) || [];
