...
export function VoiceTaskManager() {
  ...
  const { tasks, completeTask, reopenTask, updateTask, deleteTask } =
    useTasks();
    ...
    const checkForSuggestions = () => {
      ...
      if (hour === 10) {
        ...
        if (quickTasks.length >= 3) {
          sendSuggestion(
            `You could knock out ${quickTasks.length} quick tasks now to build momentum!`
          );
        }
      }
      ...
      if (hour === 17) {
        ...
        if (todayTasks.length > 0) {
          sendSuggestion(
            `You have ${todayTasks.length} tasks due today. Need a final push?`
          );
        }
      }
    };
  ...
  const stats = {
    ...
    messagesCount: conversations.reduce(
